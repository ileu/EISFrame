            ...
            tot_imp = float(
                np.mean(
                    ...
                    where=np.logical_and(
                        cycle.time - cycle.time[0] >= 3600,
                        cycle.time - cycle.time[0] <= 3610
                    )
                )
            )
...
def parameter():
    path1 = r"C:\Users\ueli\Desktop\Sauter Ulrich\Projects\EIS Tail\Data\Acetonitryle"
    ...
    for param in params:
        ...
        plt.tight_layout()
        plt.savefig(path1 + rf"\trends\trend_{param}")
