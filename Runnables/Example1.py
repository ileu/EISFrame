...
def main1():
    ...
    circuit = CustomCircuit(
        "R0-p(R1,C1)-p(R2-Wo1,C2)", initial_guess=[0.01, 0.01, 100, 0.01, 0.05, 100, 1]
    )
    bounds = (
...
def main2():
    ...
    custom_circuit = CustomCircuit(circuit, initial_guess=p.values())
    p["omega"] = 1
    ...
    res4 = timeit.timeit(
        "custom_circuit.predict(np.array([1]),True)", number=tries, globals=locals()
    )
...
def main3():
    ...
    param2 = [0.1, 1037.9, 3.416e-10, 0.9896, 1512.9, 2.697e-8, 0.920, 743.7, 2.78]
    fig, ax = create_fig()
    data.plot_nyquist(ax, cell=Cell(3, 0.7))
    print("PLOTTED")
    data.fit_nyquist(
        ax, fit_circuit=circuit2, fit_guess=param2, draw_circle=False, cell=Cell(3, 0.7)
    )
...
def main4():
    ...
    file3 = (
        r"\20200528_LLZTO_polished_Batch3-Ampcera-acetone-2h_400C"
    ...
    for file in files:
        ...
        data = load_data(path + file, sep=",")[-1]
        fig, axs = create_fig()
        data.plot_nyquist(axs)
        data.fit_nyquist(axs, circuit, param, path=path + rf"\plots\{name}_fit.txt")
